...
2. run `react-native bundle --platform android --dev false --entry-file index.js --bundle-output android/app/src/main/assets/index.android.bundle --assets-dest android/app/src/main/res`. This will create the `index.android.bundle` file in the `android/app/src/main/assets` folder and the assets in the `android/app/src/main/res` folder. Careful, the asses folder will be overwritten if it already exists. _(This step is necessary to create the assets and the bundle file for the app to work)_.
3. run `cd android && ./gradlew assembleDebug`. This will create the `app-debug.apk` file in the `android/app/build/outputs/apk/debug` folder.
4. :warning: **APP STILL IN DEV** :warning: Copy the `app-debug.apk` file to your favorite messaging app and send it to yourself _(or use a USB cable to transfer it to your device)_.
